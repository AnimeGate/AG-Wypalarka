...
ipcRenderer.on("debug:log", (_event, data: DebugLogData) => {
  // Dispatch custom event to the window with full data including window info
  ...
  // If this is a network log, also dispatch a network event for the Network tab
  if (data.level === "network" && data.args && data.args.length > 0) {
    ...
    if (networkData && networkData.id) {
      window.dispatchEvent(
        new CustomEvent("network", { detail: networkData })
      );
    }
  }
});
